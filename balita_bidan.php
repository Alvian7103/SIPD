                  ...
                  <div class="card">
                    <div class="card-body">
                      ...
                      <form method="post" action="">
                        <div class="modal-body">
                          <h5 class="text-danger">*Pastikan Data Ibu Sudah Ada</h5>
                          ...
                          <div class="row">
                            <div class="form-group col-md-2">
                              ...
                              <input type="text" maxlength="16" name="NIK_balita" placeholder="NIK Balita" class="form-control" required>
                            </div>
                            <div class="form-group col-md-5">
                              <label for="nama_balita">Nama Balita</label>
                              <input type="text" name="nama_balita" placeholder="Nama Balita" class="form-control" required>
                            </div>
                          </div>
                          <div class="row">
                            <div class="form-group col-md-2">
                              <label for="tempat_lahir_balita">Tempat Lahir</label>
                              <input type="text" name="tempat_lahir_balita" placeholder="Tempat Lahir" class="form-control" required>
                            </div>
                            <div class="form-group col-md-2">
                              <label for="tgl_lahir_balita">Tanggal Lahir</label>
                              <input type="date" max="<?= date('Y-m-d'); ?>" name="tgl_lahir_balita" placeholder="Tanggal Lahir" class="form-control" required>
                            </div>

                            <div class="form-group col-md-2">
                              <label for="jenis_kelamin">Jenis Kelamin</label>
                              <select name="jenis_kelamin" class="form-control" required>
                                <option hidden>Pilih Jenis Kelamin Balita</option>
                            ...
                            <div class="form-group col-md-2">
                              ...
                              <select name="agama_balita" class="form-control" required>
                                <option hidden>Pilih Agama Balita</option>
                          ...
                          <div class="form-group col-md-5">
                            ...
                            <select name="NIK_ibu" class="form-control" required>
                              <option hidden>Pilih Ibu</option>
                              <?php
                              include "koneksi.php";
                              $query = mysqli_query($koneksi, "SELECT * FROM ibu ORDER BY tgl_lahir_ibu DESC") or die(mysqli_error($koneksi));
                              while ($data = mysqli_fetch_array($query)) {
                                echo "<option value=\"{$data['NIK_ibu']}\">{$data['NIK_ibu']} - {$data['nama_ibu']}</option>";
                              }
                              ?>
                            </select>
                          </div>
                        ...
                        <div class="row">
                          ...
                          <div class="col-md-5">
                            <button type="submit" class="btn btn-primary w-100" name="simpan_balita">Simpan</button>
                          </div>
                          <div class="col-md-auto"></div>
                          <div class="col-md-5">
                            <a href="balita_bidan.php" type="button" class="btn btn-danger w-100">Batal</a>
                          </div>
                  ...
                  <table class="table table-bordered table-striped text-center" width="100%" display="block">
                    ...
                    ?>
                      <tr>
                        ...
                        <td>
                          <a href="ubah_balita_bidan.php?id_balita=<?= $data['id_balita'] ?>" type="button" class="btn btn-warning"><i class="fas fa-pencil-alt"></i> Ubah</a>
                        </td>
                        <td>
                          <button class="btn btn-danger" type="button" data-bs-toggle="modal" data-bs-target="#myModalHapus<?= $no ?>"><i class="fas fa-trash"></i> Hapus</button>
                        </td>
                        <td>
                          <a href="detail_balita_bidan.php?id_balita=<?= $data['id_balita'] ?>" class="btn btn-info"><i class="fa-solid fa-eye"></i> Lihat</button>
                        </td>
                      </tr>
                      <!-- The Modal Hapus -->
                            ...
                            <form method="post" action="#">
                              <div class="modal-body">
                                ...
                                <h5 class="text-center">Apakah Anda Yakin Akan Menghapus Data Ini ? <br>
                                  <span class="text-danger"><?= $data['NIK_balita'] ?> - <?= $data['nama_balita'] ?></span>
                                </h5>
...
<?php
include 'koneksi.php';
if (isset($_POST['simpan_balita'])) {
  $id_balita = $_POST['id_balita'];
  $NIK_balita = $_POST['NIK_balita'];
  $nama_balita = $_POST['nama_balita'];
  $tempat_lahir_balita = $_POST['tempat_lahir_balita'];
  $tgl_lahir_balita = $_POST['tgl_lahir_balita'];
  $jenis_kelamin = $_POST['jenis_kelamin'];
  $agama_balita = $_POST['agama_balita'];
  $NIK_ibu = $_POST['NIK_ibu'];
  $id_pengguna = $_POST['id_pengguna'];

  $cek_data = mysqli_query($koneksi, "SELECT * FROM balita WHERE NIK_balita='$NIK_balita'");
  $cek = mysqli_num_rows($cek_data);

  if ($cek > 0) {
    echo "<script>alert('Data Balita Sudah Ada, Silahkan Cek Kembali')</script>";
    echo "<script>window.location.href='balita_bidan.php'</script>";
  } else {
    $query = "INSERT INTO balita (id_balita, NIK_balita, nama_balita, tempat_lahir_balita, tgl_lahir_balita, jenis_kelamin, agama_balita, NIK_ibu, id_pengguna) VALUES('$id_balita','$NIK_balita','$nama_balita','$tempat_lahir_balita','$tgl_lahir_balita','$jenis_kelamin','$agama_balita','$NIK_ibu','$id_pengguna')";
...
if (isset($_POST['hapus'])) {
  ...
  $hapus = mysqli_query($koneksi, "DELETE FROM balita WHERE NIK_balita = '$_POST[NIK_balita]'");
  if ($hapus) {
