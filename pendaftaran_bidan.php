        ...
        <div class="sb-sidenav-menu">
          <div class="nav">
            ...
            <div class="collapse" id="pagesCollapsePelayanan" aria-labelledby="headingOne" data-bs-parent="#sidenavAccordionPages">
              <nav class="sb-sidenav-menu-nested nav">
                <a class="nav-link" href=pendaftaran_bidan.php>
                                ...
                                <form method="post">
                                  <div class="modal-body">
                                    ...
                                    <h5 class="text-center">Apakah Anda Yakin Akan Menghapus Data Ini ? <br>
                                      <span class="text-danger"><?= $data['tgl_reg'] ?> - <?= $data['NIK_balita'] ?> - <?= $data['nama_balita'] ?></span>
                                    </h5>
                                    <div class="modal-footer">
...
if (isset($_POST['simpan'])) {
  ...
  $cek_data = mysqli_query($koneksi, "SELECT * FROM registrasi WHERE NIIK_balita='$NIK_balita");
  $cek = mysqli_num_rows($cek_data);

  if ($cek > 0) {
    echo "<script>alert('Data Registrasi Sudah Ada, Silahkan Cek Kembali')</script>";
    echo "<script>window.location.href='pendaftaran_bidan.php'</script>";
  } else {
    $query = "INSERT INTO registrasi (no_reg, tgl_reg, NIK_balita, NIK_kader) VALUES('$no_reg','$tgl_reg','$NIK_balita','$NIK_kader')";
...
if (isset($_POST['ubah'])) {
  ...
  $ubah = mysqli_query($koneksi, "UPDATE registrasi SET tgl_reg = '$_POST[tgl_reg]',NIK_balita = '$_POST[NIK_balita]', NIK_kader = '$_POST[NIK_kader]' WHERE no_reg = '$_POST[no_reg]'");
  if ($ubah) {
...
if (isset($_POST['hapus'])) {
  ...
  $hapus = mysqli_query($koneksi, "DELETE FROM registrasi WHERE no_reg = '$_POST[no_reg]'");
  if ($hapus) {
